...
class Bannerurl_model extends CI_Model {   
		...
		public function get_fieldDetails_by_join(){
			 $this->db->select('bmf.*');
			 $this->db->from('tbl_bannerurl_masterfields bmf');
			 $this->db->join('tbl_bannerurl_field_options bfo','bfo.country = c.id','left');
		...
		public function get_bannerurl_fieldsByCatgId($catgId,$whereCondition=NULL){
		     $this->db->select('mct.*, mfo.*');
			 $this->db->from('tbl_bannerurl_mastercategory mct');
			 $this->db->join('tbl_bannerurl_masterfields mfo','mfo.bannerurlCategoryId = mct.id','inner');
		...
		public function filter_bannerurl_rowhead($catgId){
			 $fields	=	$this->get_bannerurl_fieldsByCatgId($catgId);			 
			 $rowheadings = array();
			 if(!empty($fields)){
		...
		public function field_options($fieldId){
			$this->db->select('tbfo.*');	
			$this->db->from('tbl_bannerurl_field_options tbfo');
