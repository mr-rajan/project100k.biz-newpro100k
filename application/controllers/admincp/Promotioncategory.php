...
class Promotioncategory extends CI_Controller{
		...
		public function __construct(){
			...
			$this->load->model('admincp/promotion_model');				
		}
		...
		public function index($catgId){	
			...
			if($this->session->userdata('post_promotionCommonForm_Data'))
			$this->session->unset_userdata('post_promotionCommonForm_Data');	
			$data['request_catgId']				    =	$catgId;
			$decryptedCatgId						=	$this->encrypt->decode(str_replace(array('-', '_', '~'),array('+', '/', '='),$catgId));
			$categoryRecordInfo						=	$this->base_model->is_Record_Exists('tbl_promotion_mastercategory',array('id','title'),array('id'=>$decryptedCatgId));
			...
			if(!empty($categoryRecordInfo)){
				...
				$data['promotionCategoryTableHeaders']	=	$this->promotion_model->filter_promotion_rowhead($decryptedCatgId);
				if(!empty($data['promotionCategoryTableHeaders'])){
					$sortableArray	=	array();
					foreach($data['promotionCategoryTableHeaders'] as $key=>$value){
						 if($value['sortable'] == 'no')array_push($sortableArray,($key+1));						
					}
					$sortableArray[]				=	count($data['promotionCategoryTableHeaders'])+2;
					$data['unSortableColumnKeys']   =	implode(',',$sortableArray);
					if($this->db->table_exists('dynamic_promo_tbl_'.$tblName))
					$data['ads_lists']				=	$this->base_model->get_All_Records('dynamic_promo_tbl_'.$tblName,'','','id','asc');					
					
				}
				
				$this->load->view('templates/admincp/header', $data);
				$this->load->view('admincp/promotioncategory/index', $data);	
			}
			else{			
		...
		public function category($catgId)
		{
			...
			$data['title'] 							    = 	'Projects 100K';						
			$categoryInfo							    =	$this->base_model->is_Record_Exists('tbl_promotion_mastercategory',
		...
		public function addnew($catgId){
			...
			if($this->session->userdata('post_promotionCommonForm_Data'))
			$data['postedRequestValue'] = 	$this->session->userdata('post_promotionCommonForm_Data');
			$data['hdn_editable_formID']=	'';	
			$data['promotionInfo']		=	array();
			$data['ref_hdncatgID']		=	$catgId;
			$decryptedCatgId			=	$this->encrypt->decode(str_replace(array('-', '_', '~'),array('+', '/', '='),$catgId));
			$categoryInfo				=	$this->base_model->is_Record_Exists('tbl_promotion_mastercategory',array('title'),array('id'=>$decryptedCatgId));
			$data['breadcrumb_string']	=	'<a href="'.base_url().'admincp/promotioncategory/'.$catgId.'">'.$categoryInfo->title.' Listings</a>';
			
			$fields						=	$this->promotion_model->get_promotion_fieldsByCatgId($decryptedCatgId);
			if(!empty($fields)) {
				foreach($fields as $key=>$field){
					if($field['type']	== 3 || $field['type']	== 4 || $field['type']	== 5){
							$options			=	$this->promotion_model->field_options($field['id']);							
			...
			$this->load->view('templates/admincp/header', $data);
			$this->load->view('admincp/promotioncategory/form', $data);
		}
		...
		public function edit($catgSection,$editableAdsId){
			...
			$categoryRecordInfo			=	$this->base_model->is_Record_Exists('tbl_promotion_mastercategory',array('id','title'),array('uri_string'=>$catgTitle));			
			$data['ref_hdncatgID']		=	str_replace(array('+', '/', '='), array('-', '_', '~'),$this->encrypt->encode($categoryRecordInfo->id));
			$data['breadcrumb_string']	=	'<a href="'.base_url().'admincp/promotioncategory/'.$data['ref_hdncatgID'].'">'.$categoryRecordInfo->title.' Listings</a>';
			$decryptedEditable_formID	=	$this->encrypt->decode(str_replace(array('-', '_', '~'),array('+', '/', '='),$editableAdsId));
			$data['postedRequestValue']	=	(array)$this->base_model->is_Record_Exists('dynamic_promo_tbl_'.$catgSection,'',array('id'=>$decryptedEditable_formID));	
			$fields						=	$this->promotion_model->get_promotion_fieldsByCatgId($categoryRecordInfo->id);
			//if($this->session->userdata('post_promotionCommonForm_Data'))
			//$data['postedRequestValue'] = 	$this->session->userdata('post_promotionCommonForm_Data');
			if(!empty($fields)) {
				foreach($fields as $key=>$field){
		...
		public function save(){
			...
			$categoryInfo	 =	$this->base_model->is_Record_Exists('tbl_promotion_mastercategory', array('*'), array('id'=>$decryptedCatgId));
			$hdCatgTitle	 =	strtolower(str_replace(array(' ','-'),array('_'),$categoryInfo->title));
			if(!empty($categoryInfo)){				
				$fields					=	$this->promotion_model->get_promotion_fieldsByCatgId($categoryInfo->id,array('isMandatory'=>'yes'));			
				...
				foreach($mandatoryFields_array as $key=>$mandatoryField){	
					...
					if($fields[$key]['type']	==	6){
							...
							if($editable_formID!=''){
								...
								$editable_RecordInfo =	$this->base_model->is_Record_Exists('dynamic_promo_tbl_'.$hdCatgTitle, array($imageFieldName), 
														array('id'=>$editable_formID));
					...
					elseif($fields[$key]['type']	==	7){
						//echo '--hyperlink_label_'.$fields[$key]['id'];die;
						$this->form_validation->set_rules($fieldName, $mandatoryField, 
						'trim|required|regx_match[/^(http\:\/\/|https\:\/\/)?([a-z0-9][a-z0-9\-]*\.)+[a-z0-9][a-z0-9\-]*$/]|xss_clean');
						$this->form_validation->set_rules('hyperlink_label_'.$fields[$key]['id'], $mandatoryField.' label ', 'trim|required|xss_clean');	
					}
				...
				{
					$this->session->set_flashdata('admincp_promotion_common_form_flash_message', '<div class="alert alert-danger alert-dismissible">
									<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.validation_errors().'</div>');
						
						$this->session->set_userdata('post_promotionCommonForm_Data',$this->input->post());				
						if($editable_formID!='')
							redirect('admincp/promotioncategory/edit/'.$hdCatgTitle.'/'.$this->input->post('hdn_editable_formID'));
						else	
							redirect('admincp/promotioncategory/addnew/'.$this->input->post('ref_hdncatgID'));
				}
				else{
							...
							$fields					=	$this->promotion_model->get_promotion_fieldsByCatgId($categoryInfo->id);	
								...
								$fieldName	=	strtolower(str_replace(array(' ','-'),array('_'),$field['title']));	
									...
									if($fieldName != $imageFieldName){
										...
										elseif($field['type'] == 7){
												...
												$userData['hyperlink_label_'.$field['id']] =	$this->security->xss_clean($this->input->post('hyperlink_label_'.$field['id']));
										}
										else{
												$userData[$fieldName]	=	htmlentities($this->security->xss_clean($this->input->post($fieldName)));
										}
							...
							if($editable_formID == ''){
									...
									$userData['promotionCategoryId']	=	$decryptedCatgId;
									$dataStatus	=	$this->base_model->saveRecord($userData,'dynamic_promo_tbl_'.$hdCatgTitle);
									...
									$dataStatus	=	$this->base_model->updateRecord($userData,'dynamic_promo_tbl_'.$hdCatgTitle,array('id'=>$editable_formID));
							...
							if($dataStatus){
									...
									{
										$this->session->set_flashdata('admincp_promotion_common_form_flash_message', '<div class="alert alert-success alert-dismissible">
										...
										redirect('admincp/promotioncategory/edit/'.$hdCatgTitle.'/'.$encryptedID);
									}
									elseif($this->input->post('submit')=='save')
									{
										$this->session->set_flashdata('admincp_promotioncategory_indexpage_flash_message', '<div class="alert alert-success alert-dismissible">
												<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
												<i class="icon fa fa-check"></i> '.$flash_message.'</div>');	
										redirect('admincp/promotioncategory/'.$this->input->post('ref_hdncatgID'));
									}
							}
							else{
