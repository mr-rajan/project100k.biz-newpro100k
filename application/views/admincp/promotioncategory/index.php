 <?php $this->load->view('templates/admincp/leftsidebar')?>
  ...
  <div class="content-wrapper">
    ...
    <?php if($this->session->flashdata('admincp_promotioncategory_indexpage_flash_message')) { 
				echo '<div id="sessionMessageDiv" style="padding:10px;">'.$this->session->flashdata('admincp_promotioncategory_indexpage_flash_message').'</div>';
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="<?php echo base_url().'admincp/promotion'?>">Cateogory Lists</a></li>
      </ol>
    </section>
<style>
...
<div id="overlay">	
 ...
 <?php	  $rowHeads_array = array();
		  if(!empty($promotionCategoryTableHeaders)):
			  $rowHeads_array	=	array(array('fldName'=>'','rowHeadTitle'=>'SNo'));
			  foreach($promotionCategoryTableHeaders as $key=>$tableHeader):	
				 $fieldType	=	$this->base_model->is_Record_Exists('tbl_promotion_masterfields',array('id','type'),
																	array('title'=>$tableHeader['headingTitle'], 
																   'promotionCategoryId'=>$decryptedCatgId));
...
?>
            ...
            <!-- /.box-header -->
            <div class="box-body">
              <table id="promotiondynamic_catg"  class="table table-striped table-bordered dt-responsive nowrap" width="100%" cellspacing="0">
                <thead>
                <tr>
						 ...
						 echo '<tr id="row_'.$listValue->id.'">';
								for($i=0;$i<count($rowHeads_array);$i++)
								{
									...
									elseif($i==(count($rowHeads_array)-1)){
										...
										$edit	=	'<a href="'.base_url().'admincp/promotioncategory/edit/'.$tblName.'/'.$link.'" alt="Edit" title="Edit">
													 <i class="fa fa-edit" aria-hidden="true"></i></a>';
										$delete	=	'<a href="javascript:void(0);" 
													 onclick="delete_record('.$listValue->id.',\'promotioncategoryads\')" alt="Delete" title="Delete">
										...
										{													
													  case 3: 
																$optionInfo	=	$this->base_model->is_Record_Exists('tbl_promotionurl_field_options',
																									array('optionvalue'),
          ...
          <input type="hidden" id="total_rowheadcount" value="<?php echo count($promotionCategoryTableHeaders)?>">
...
<?php $this->load->view('templates/footer_js_script')?>
 ...
 $(document).ready(function(e) { 
  var promotionurl_rows_selected = []; //alert($('#sortableColumnKeys').val());
	var table = $('#promotiondynamic_catg').DataTable({
      "paging": true,
	  "responsive":true,
      "lengthChange": true,      
      "searching": true,
	  ...
	  "buttons": [
	  			 {
					...
					"action":function( e, dt, node, config){window.location.href=$.trim($('#baseURL').val())+'admincp/promotioncategory/addnew/'+$.trim($('#request_catgId').val());}
	  			  },
				  'colvis',
	  			{
					"extend":'collection',
				...
				},	    	  	
	  			{
					...
					if(promotionurl_rows_selected.toString()!=''){
								if(confirm('Do you really want to delete selected record(s)?')){									
									$.ajax({ 
											'type':'post',
											'url': $.trim($('#baseURL').val())+'admincp/action/process',
											'data': {'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>', 
													'action':'delete', 'controller':'promotioncategory', 'ids':promotionurl_rows_selected.toString(), 'tbl':'<?php echo $tblName?>'},
					...
					if(promotionurl_rows_selected.toString()!=''){
								if(confirm('Are you sure to activate selected record(s)?')){									
									$.ajax({ 
											...
											'url': $.trim($('#baseURL').val())+'admincp/action/process',
											'data': {'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>', 
													'action':'active', 'controller':'promotioncategory', 'ids':promotionurl_rows_selected.toString(), 'tbl':'<?php echo $tblName?>'},
											'beforeSend': function(){ $('#overlay').fadeIn('slow');},
											'success':function(respData){
												if(respData!='')window.location.reload();
											}
					...
					if(promotionurl_rows_selected.toString()!=''){
								if(confirm('Are you sure to suspend selected record(s)?')){									
									$.ajax({ 
											...
											'url': $.trim($('#baseURL').val())+'admincp/action/process',
											'data': {'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>', 
													'action':'suspend', 'controller':'promotioncategory', 'ids':promotionurl_rows_selected.toString(), 'tbl':'<?php echo $tblName?>'},
											'beforeSend': function(){ $('#overlay').fadeIn('slow');},
											'success':function(respData){ 															
															if(respData!='')window.location.reload();
											}
	...
	$('#promotiondynamic_catg tbody').on( 'click', 'tr', function () {		 
      ...
      var index = $.inArray(rowId[1], promotionurl_rows_selected);
	 
      // If checkbox is checked and row ID is not in list of selected row IDs
      if($(this).hasClass('selected') && index === -1){
         promotionurl_rows_selected.push(rowId[1]);
      // Otherwise, if checkbox is not checked and row ID is in list of selected row IDs
      } else if (!$(this).hasClass('selected') && index !== -1){
         promotionurl_rows_selected.splice(index, 1);
      }
