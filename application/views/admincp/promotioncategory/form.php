      ...
      <!-- Your Page Content Here -->
       <!-- form start -->
            <?php   if(!empty($fields)):  					  
				      ...
				      echo form_open_multipart('admincp/promotioncategory/save',$form_attributes);
			  ?>
        <div class="box box-info">
       		  <?php if($this->session->flashdata('admincp_promotion_common_form_flash_message')) {
             	echo '<div id="sessionMessageDiv" style="padding:10px;">'.$this->session->flashdata('admincp_promotion_common_form_flash_message').'</div>';
			 } 			 	
			 ?>
            <div class="box-header with-border">
                    <button class="btn btn-info pull-right" type="button" name="cancel" value="cancel" 
                    onclick="javascript:window.location.href='<?php echo base_url().'admincp/promotioncategory/'.$ref_hdncatgID?>'">Cancel</button>
            ...
            <!-- /.box-header -->
              ...
              <div class="box-body">
                  <?php foreach($fields as $key=>$field):?>
                         ...
                         <?php ($field['type']=='7') ? $linklabel_style = '' :  $linklabel_style = 'style="display:none;"';?>
                            <div class="form-group has-feedback" id="linklabel_div" <?php echo $linklabel_style?>>
                          <label for="title" class="col-sm-2 control-label"><?php echo $field['title'].' Label ';if($linklabel_style==''):?><span class="required">*</span><?php endif;?></label>
                          <div class="col-sm-10">
                            <?php 
							(!empty($postedRequestValue) && array_key_exists('hyperlink_label_'.$field['id'],$postedRequestValue) 
							&& $postedRequestValue['hyperlink_label_'.$field['id']]!='') ? $linkvalue = $postedRequestValue['hyperlink_label_'.$field['id']] : 
							$linkvalue	= '';
							$data = array('name'=>'hyperlink_label_'.$field['id'], 'id'=>'hyperlink_label_'.$field['id'], 
                                                'value'=>$linkvalue, 'class'=>'form-control', 'placeholder'=>'Hyperlink Label');
                                  // if($linklabel_style=='')
								  // $data['required']	=	'required';
                                   echo  form_input($data)
                            ?>                    
