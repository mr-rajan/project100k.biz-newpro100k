...
.has-feedback .form-control{
	padding-right: 23px !important;
      ...
      <h1><?php echo (!empty($bannerUrlFieldInfo) && $ref_hdnBannerUrlID!='') ? 'Edit ': 'Add New '?>Field</h1>
      
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i>Home</a></li>
        <li><a href="<?php echo base_url().'admincp/bannerurl'?>">Url &amp; Banner Listing</a></li>
        <li><?php echo (!empty($bannerUrlFieldInfo) && $ref_hdnBannerUrlID!='') ? 'Edit ': 'Add New '?>Field</li>
      </ol>
    </section>
      ...
      <!-- Your Page Content Here -->
       <!-- form start -->
            <?php   $form_attributes = array('name'=>'bannerURLForm','class'=>'form-horizontal' );
				      echo form_open_multipart('admincp/bannerurl/savefield',$form_attributes);
			  ?>
        <div class="box box-info">
       		  <?php if($this->session->flashdata('admincp_bannerurlfieldform_flash_message')) {
             	echo '<div id="sessionMessageDiv" style="padding:10px;">'.$this->session->flashdata('admincp_bannerurlfieldform_flash_message').'</div>';
			 } 
			 
			 ?>
            <div class="box-header with-border">
                    <button class="btn btn-info pull-right" type="button" name="cancel" value="cancel" 
                    onclick="javascript:window.location.href='<?php echo base_url().'admincp/bannerurl/edit/'.$ref_hdnBannerUrlID?>'">Cancel</button>
            ...
            <!-- /.box-header -->
            <?php
            		$postedValue_title			=	$postedValue_type	=	'';
					...
					if(!empty($bannerUrlFieldInfo)){
							$postedValue_title			=	$bannerUrlFieldInfo->title;
							$postedValue_type			=	$bannerUrlFieldInfo->type;
							$postedValue_isMandatory	=	$bannerUrlFieldInfo->isMandatory;
							$postedValue_Status			=	$bannerUrlFieldInfo->status;
							$addtoTableHeading			=	$bannerUrlFieldInfo->addtoTableHeading;
							$postedValue_fieldOptions	=	$fieldOptions;
							$postedValue_order			=	$bannerUrlFieldInfo->order;
							$postedValue_Sortable 		= 	$bannerUrlFieldInfo->sortable;
            ...
            <div class="controlsFirst"> 
                   ...
                   <?php   foreach($postedValue_fieldOptions as $key=>$optVal):?>
                    <div class="entry input-group col-xs-8">
                        <input class="form-control" name="field_options[]" value="<?php echo $optVal->optionvalue?>" type="text" placeholder="Type something" required="required"/>
                 ...
                 <?php	endif;?> 
                 <?php  if(!empty($bannerUrlFieldInfo) && ($postedValue_type=='3'|| $postedValue_type=='4'||$postedValue_type=='5') && empty($postedValue_fieldOptions))
              ...
              <!-- /.box-footer -->
               <input type="hidden" name="ref_hdnBannerUrlID" id="ref_hdnBannerUrlID" value="<?php echo $ref_hdnBannerUrlID?>">
...
$('select[name="type"]').bind('change',function(e){
		if($(this).val() == '3' || $(this).val() == '4' || $(this).val() == '5' ){
			...
			$('#form').find('input[name="field_options[]"]').attr('required',true);
			 ...
			 $('#form').find('input[name="field_options[]"]').remove('required');
			  $('#options').hide();
		}
